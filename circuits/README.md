...
| `circuit-rwa-asset` | `circuits/rwa-asset` | RWA chain logic: `apply_issue` (self-mint by the designated issuer) and `apply_compliant_transfer` (requires both sender and recipient have `AccountEntry.identity_hash` set — the KYC/allowlist marker — before delegating the actual balance/nonce math to `circuit_account::apply_transfer`). Also the prospectus-style limits an issuer sets with `SetAssetLimits`: investor cap (`max_holders`, tracked in `Asset.holder_count`), per-holder concentration limit, attestation max age, and self-expiring lock-ups (`LockHolderAmountUntil`). Proof that circuits compose: this crate reuses `circuit-account` and `circuit-identity` rather than reimplementing them. |
| `circuit-identity`  | `circuits/identity`  | The attestor registry and attestations: `apply_register_attestor`/`apply_deregister_attestor`, `apply_grant_attestation`/`apply_revoke_attestation`, `is_attested` (live attestation = `identity_hash` from an attestor still in the registry), and `apply_verify_credential` (Groth16 proof from `circuit-identity-zk`, bound to the sender). Which address may register attestors (`AdminRole::Attestor`) stays in the runtime — that's chain config, not identity logic. |
| `circuit-identity-zk` | `circuits/identity-zk` | The zk credential circuit itself (Poseidon preimage, Groth16 over BLS12-381) plus the checked-in devnet keys. Pure crypto, no state access; `circuit-identity` is its only consumer. |
| `circuit-staking`   | `circuits/staking`   | Stake/unstake/unbonding, block reward and the fee split (30% proposer / 20% treasury / 50% burned by omission), equivocation and downtime slashing. |
| `circuit-governance` | `circuits/governance` | Validator governance: `apply_submit`/`apply_vote`/`apply_execute` over a `GovernanceAction` — `SetChainParams`, `SetAdmin` (rotates an `AdminKey`), `TreasurySpend` (the only way out of `treasury_account()`). Voting power is the set's `VotingPower`; passes on yes > no and yes ≥ `ChainParams.proposal_quorum_bps`. Writes raw `CF_GOVERNANCE` rows via `GovernanceUpdates`. |
