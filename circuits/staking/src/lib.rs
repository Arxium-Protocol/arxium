...
pub fn apply_stake<V: KvRead<Error = StorageError>>(
    ...
    let mut master_entry = view
        .get(&AccountKey(master))?
        .unwrap_or_else(default_account);
    if nonce != master_entry.nonce {
        return Err(StakingError::InvalidNonce {
            master: master.clone(),
            expected: master_entry.nonce,
            got: nonce,
        });
    }
    ...
    if masters.iter().any(|m| m != master) {
        return Err(StakingError::ValidatorHasOtherMaster {
    ...
    {
        return Err(StakingError::AlreadyUnbonding {
            master: master.clone(),
            validator: validator.clone(),
        });
    }
    ...
    if new_total > MAX_DELEGATION_PER_VALIDATOR {
        return Err(StakingError::DelegationCapExceeded {
            validator: validator.clone(),
            new_total,
        });
    }
    ...
    let mut sub_entry = view
        .get(&AccountKey(&sub_account))?
        .unwrap_or_else(default_account);
    ...
    stake_updates
        .allocations
        .insert((master.clone(), validator.clone()), Some(new_allocation));
    stake_updates
        .validator_index
        .insert(validator.clone(), vec![master.clone()]);
...
pub fn apply_unstake<V: KvRead<Error = StorageError>>(
    ...
    let mut existing = view.get(&StakeKey { master, validator })?.ok_or_else(|| {
        StakingError::NoActiveAllocation {
            master: master.clone(),
            validator: validator.clone(),
        }
    })?;
    ...
    existing.active_amount -= amount;
    existing.unbonding = Some(Unbonding {
        amount,
        unlock_at_height: now_height + unbonding_blocks,
    });
    existing.updated_at = now_height;
    master_entry.nonce += 1;
    ...
    stake_updates
        .allocations
        .insert((master.clone(), validator.clone()), Some(existing));
...
pub fn apply_slash<V: KvRead<Error = StorageError>>(
    ...
    let master = masters
        .first()
        .cloned()
        .ok_or_else(|| StakingError::NoStakeForValidator {
            validator: validator.clone(),
        })?;
    ...
    if existing.active_amount == 0 && existing.unbonding.is_none() {
        stake_updates
            .allocations
            .insert((master.clone(), validator.clone()), None);
        stake_updates
            .validator_index
            .insert(validator.clone(), Vec::new());
    } else {
        stake_updates
            .allocations
            .insert((master, validator.clone()), Some(existing));
    }
...
pub fn apply_downtime_slash<V: KvRead<Error = StorageError>>(
    ...
    let Some(existing) = view.get(&StakeKey {
        master,
        validator: primary,
    })?
    else {
    ...
    for mut allocation in due {
        ...
        if allocation.active_amount == 0 {
            stake_updates.allocations.insert(
                (allocation.master.clone(), allocation.validator.clone()),
                None,
            );
            stake_updates
                .validator_index
                .insert(allocation.validator.clone(), Vec::new());
        } else {
...
mod tests {
    ...
    fn stake_debits_master_and_credits_subaccount_by_the_same_amount() {
        ...
        let (accounts, stakes) = apply_stake(&db, &master, 0, &validator, 400, 10).unwrap();
        ...
        assert_eq!(
            db.get_stakes_by_validator(&validator).unwrap(),
            vec![master]
        );
    }
    ...
    fn stake_rejects_a_second_master_for_an_already_controlled_validator() {
        ...
        let (accounts, stakes) = apply_stake(&db, &alice, 0, &validator, 100, 10).unwrap();
        commit(&db, accounts, stakes);

        let err = apply_stake(&db, &bob, 0, &validator, 100, 10).unwrap_err();
    ...
    fn partial_unstake_splits_active_and_unbonding_in_the_same_record() {
        ...
        let (accounts, stakes) =
            apply_unstake(&db, &master, 1, &validator, 200, 50, UNBONDING_BLOCKS).unwrap();
        commit(&db, accounts, stakes);
        ...
        let allocation = db
            ...
            .unwrap();
        assert_eq!(
            allocation.active_amount, 300,
            "remaining stake must stay active"
        );
        let unbonding = allocation
            .unbonding
            .expect("requested amount must be unbonding");
    ...
    fn unstake_rejects_a_second_batch_while_one_is_already_unbonding() {
        ...
        let err = apply_unstake(&db, &master, 2, &validator, 100, 3, UNBONDING_BLOCKS).unwrap_err();
    ...
    fn slash_hits_both_active_and_unbonding_allocations() {
        ...
        let (accounts, stakes) =
            apply_slash(&db, &validator, 350, SlashReason::DoubleSign, 3).unwrap();
        commit(&db, accounts, stakes);
        ...
        assert_eq!(
            ...
            "active portion slashed first, fully consumed"
        );
        assert_eq!(
            allocation.unbonding.unwrap().amount,
            150,
            "remainder spills into unbonding"
        );
        let sub = stake_subaccount(&validator);
        assert_eq!(db.get_account(&sub).unwrap().unwrap().balance, 150);
        assert_eq!(
            db.get_account(&reward_pool_account())
                .unwrap()
                .unwrap()
                .balance,
            350,
            "slash lands in the pool"
        );
    }
    ...
    fn slash_moves_the_full_amount_to_the_reward_pool() {
        ...
        let (accounts, stakes) =
            apply_slash(&db, &validator, 120, SlashReason::ExecutionFault, 3).unwrap();
    ...
    fn slash_removes_the_allocation_once_it_nets_to_zero() {
        ...
        let (accounts, stakes) =
            apply_slash(&db, &validator, 500, SlashReason::Downtime, 2).unwrap();
        commit(&db, accounts, stakes);

        assert!(
            db.get_stake_allocation(&master, &validator)
                .unwrap()
                .is_none()
        );
        assert_eq!(
    ...
    fn total_supply_is_conserved_across_stake_and_unstake_and_only_drops_on_slash() {
        ...
        let supply = |db: &ArxiumDb| {
            db.get_account(&master).unwrap().unwrap().balance
                + db.get_account(&stake_subaccount(&validator))
                    .unwrap()
                    .map(|a| a.balance)
                    .unwrap_or(0)
                + db.get_account(&reward_pool_account())
        ...
        let (accounts, stakes) = apply_stake(&db, &master, 0, &validator, 600, 1).unwrap();
        commit(&db, accounts, stakes);
        ...
        assert_eq!(
            supply(&db),
            1000,
            "unstake request alone must not move funds yet"
        );

        let (accounts, stakes) =
            apply_slash(&db, &validator, 150, SlashReason::DoubleSign, 3).unwrap();
        commit(&db, accounts, stakes);
        assert_eq!(
            supply(&db),
            1000,
        ...
        assert_eq!(
            supply(&db),
            1000,
            "unbonding resolution moves funds within the same supply"
        );
    ...
    fn stake_rejects_delegation_over_the_cap() {
        ...
        let err = apply_stake(&db, &master, 1, &validator, 1, 11).unwrap_err();
        assert!(matches!(err, StakingError::DelegationCapExceeded { .. }));
    }
    ...
    fn downtime_slash_is_a_noop_when_primary_matches_actual() {
        ...
        let sub = stake_subaccount(&validator);
        assert_eq!(
            db.get_account(&sub).unwrap().unwrap().balance,
            500,
            "untouched"
        );
    }
