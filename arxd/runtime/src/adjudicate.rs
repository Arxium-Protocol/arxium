...
//! `Fault::BlockDivergence` — the chain-specific counterpart to
//! `xc_artifact::verify()`, which can confirm either fault is well-formed
//! (claims are validly signed, proofs verify against their claimed
//! pre-state, and the two post-roots genuinely differ) but can't say *who's*
//! wrong. That means decoding the action bytes as CoreChain's `ActionPayload`
//! and replaying them, which is what this module does.
...
//!
//! Lives in `arxd-runtime` itself (not `arx-verify`, which only depends on
//! this crate, never the reverse) for two reasons: [`ActionPayload::SubmitExecutionFault`]
//! needs to run the exact same replay on-chain to decide who to slash, and
//! `arx-verify`'s `core-adjudicate` feature re-exports these same functions
//! rather than keeping a second copy — one implementation, two callers.
//!
//! ## Coverage
//!
...
//!   in `CF_EVIDENCE`.
//! - The governor key (`RegisterAttestor`/`DeregisterAttestor`) lives in
//!   `CF_META` — only asset *balances* (`AssetBalanceKey`) and the asset
//!   registry (`AssetKey`) are Merkleized.
...
//! (a schema change, like the `CF_ASSETS`/`CF_ATTESTORS`/`CF_EVIDENCE` bumps
//! already in this codebase's history), not writing more code here.
...
/// (`validator_change`, `evidence`, `bls_key`, `operator`) never touches the
/// root, so there's nothing to `apply` for it here. `asset_registration` and
/// `attestor_registration`/`attestor_deregistration` *are* Merkleized, so
/// both are flattened below.
...
/// closed (`StorageError::UnprovenRead`) for any key outside the Merkleized
/// CFs *before* even consulting the trie: a `CF_META` key was never
/// inserted into the real trie to begin with, so a proof-backed trie
