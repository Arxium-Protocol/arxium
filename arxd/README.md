...
| `arxd/runtime/`    | The `runtime` crate. Detects equivocation — a validator signing two different blocks at one height — verifies the evidence, and applies the resulting slash. Reacts to a single `RuntimeEvent` today; more runtime responsibilities (state-root registry, cross-chain conflict resolution) land here as they are built.                    |
